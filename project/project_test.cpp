...
void print_list_energy(din16_t list_energy[GLOBAL_ETA * GLOBAL_PHI], int offset, int max_display=1000) {
    int display_count = 0;
    for (int i = 0; i < GLOBAL_ETA * GLOBAL_PHI; i++) {
        display_count++;
        if (display_count <= max_display) {
            std::cout << list_energy[i] << " ";
        }
...
int main() {
    ...
    din16_t list_3x3_energy[GLOBAL_ETA * GLOBAL_PHI]; // 3 x 3 Cluster Energy
    din16_t list_5x5_energy[GLOBAL_ETA * GLOBAL_PHI]; // 5 x 5 Cluster Energy
    din16_t list_7x7_energy[GLOBAL_ETA * GLOBAL_PHI]; // 7 x 7 Cluster Energy
    ...
    for (size_t global_eta = 0; global_eta < GLOBAL_ETA; global_eta++) {
        for (size_t global_phi = 0; global_phi < GLOBAL_PHI; global_phi++) {
            ...
            list_3x3_energy[(global_eta * CRYSTALS_ETA) + global_phi] = 0;
            list_5x5_energy[(global_eta * CRYSTALS_ETA) + global_phi] = 0;
            list_7x7_energy[(global_eta * CRYSTALS_ETA) + global_phi] = 0;
        }
    }

    // Call the project function
    project(crystal_energy, tower_energy, list_3x3_energy, list_5x5_energy, list_7x7_energy);
    input_file.close();
    ...
    print_list_energy(list_3x3_energy, 0);
    std::cout << "Sorted 5x5 Cluster Energy: " << std::endl;
    print_list_energy(list_5x5_energy, 0);
    std::cout << "Sorted 7x7 Cluster Energy: " << std::endl;
    print_list_energy(list_7x7_energy, 0);
}